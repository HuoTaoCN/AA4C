...
| **V0.8** | **Focus** | — | **收缩到「连接」：下载 / 归档与 AI 插件化 + 连接可见化 + 重建 UI + 真机验证** | 🔨 进行中（F1–F3 已完成，**F4 真机验证待用户**） |
...
> 设计详见 [TRUST_DESIGN.md](TRUST_DESIGN.md)（里程碑 R1–R4）。**里程碑 R1–R4 全部已实现**：R1 IPv6 双栈（§7.2）、R2 信任传递 / 引荐（线路见 [PROTOCOL.md](PROTOCOL.md) §18，落地见 §7.1）、R3 UPnP 自动端口映射（§7.3，只做 UPnP IGD，NAT-PMP/PCP 明确后置）、R4 桌面端内置 server 模式（§7.4）。三条前提已核实：Tailscale / ZeroTier **都需要账号或自建控制面**，真正无账号的是 Syncthing / EasyTier 那一类；「零第三方」做不到 NAT 穿透，目标应是「不依赖第三方**服务商**」；我们只要一条应用层连接，不需要照搬 VPN 式组网。

---

## V0.8 — Focus：收缩到「连接」

**目标：让代码里的 AA4C 和文档里的 AA4C 是同一个东西。**

PROJECT_VISION 与 AGENTS 都写着「不是下载器、BT 工具、云盘、同步工具」，但到 v0.7 为止，
下载中心与归档/AI 占了 **27/60 个 Tauri 命令、18/28 个设置项、一半前端代码、36% 的 store**，
`Core` 有三分之一的编排代码为它们服务。

> **编号说明**：本阶段用 F1–F5，不用 R——R1–R4 已经是 V0.7「Trust / Reach」的里程碑编号，
> 同一份文档里一个代号两个意思会出事。ARCHITECTURE 承诺了三个版本的 `Plugin` trait
一行都没写。

核心竞争力只有一条，而且没有对手正面解决：**不注册账号、不依赖第三方服务商，
让「我的几台设备」在任何网络下自己连成一片**（Tailscale/ZeroTier 要账号 + 厂商控制面，
Syncthing 依赖公共发现服务器且只做同步，LocalSend 只有局域网）。

- **F1 插件化**：`aa4c-plugin` 定义边界；下载、归档与 AI 移到边界外；`aa4c-core` 的
  依赖列表里不再有它们。**已完成**——core 6533→4419 行、命令 60→35。
- **F2 连接可见化**：连接阶梯目前算完就扔（`ConnectionVia` 只有三个值、`resolve_addr`
  丢掉了来源）。把它变成一等数据，让核心竞争力在界面上看得见。
- **F3 重建 UI**：设计系统（2100 行散装 CSS → 共享组件）、换掉 emoji、主导航 8→4、
  首页从功能宫格改成设备网络状态图、设置页 903→300 行。
- **F4 真机验证**：照 [docs/V0.7_VERIFICATION.md](docs/V0.7_VERIFICATION.md) 走 A/B/C 三组。
- **F5 补深度**：分享链接 deep-link + 二维码、按需拉取续传、打洞成功率。

**明确不做**：不重写（代码质量不差，问题是范围）、不删已发布能力（所以是插件化不是删除）、
不做动态加载插件（Rust 无稳定 ABI）。
