...
/// 没装插件的构建（本文件全部用例都是这个状态——空的 `PluginRegistry`）调用插件
...
/// 都成立，不再只是下载。
#[tokio::test]
async fn calling_a_plugin_that_is_not_installed_reports_unavailable() {
    ...
    for (plugin, method) in [
        ...
        let err = node
            .core
            .plugin_invoke(plugin, method.into(), serde_json::json!({}))
            .await
            .unwrap_err();
        assert_eq!(
            err.code(),
            "unavailable",
            "{plugin}.{method} should report unavailable, got {err:?}"
        );
    }

    // 装了什么插件也要能查——前端靠它决定「更多」分区显示什么。
    assert!(node.core.plugin_manifest().is_empty());
