...
/// 与 aria2c 真正 bind 之间存在竞态窗口（aria2 不支持"由 OS 自选端口"），
/// 端口被抢由调用方的重试逻辑兜底（见 `lib.rs`），这里不做任何预留尝试。
...
/// 生成一个高熵密钥：两个 UUID v4 拼成 32 字节 base58 编码——不为此单独引入
/// 新的 RNG 依赖，同 C6 `generate_token` 的先例。
