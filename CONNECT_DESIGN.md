...
- 好友设备用他自己的服务器 → **向对端的 home server 发起 Lookup**（`resolve_addr` 新增的
  一档，PROTOCOL.md §17，V0.3 遗留 gap 已补完）——**地址查询**这部分已实现；打洞/中继
  **信令**目前仍只会打向本机自己配置的服务器，不会打向对端的 home server（两台互不知情
  的独立服务器之间没有公共撮合点，真正的跨服务器信令联邦需要服务器间协议，是独立的、
  后置的项目，见 §12「多服务器联邦」）——这一点与本节最初的设计意图（打洞/中继信令也
  发生在被叫方服务器上）有出入，是已知的实现收敛，不是漏做。
- 对端 home server 地址（含指纹）在**配对时交换并落库**（`PairServerHint`，proto ≥ 5），
  之后不做持续刷新（同 `last_addr` 的新鲜度惯例）——这是 `devices` 表的正式字段（§8）。
- 中继会话目前仍使用**本机自己配置**的服务器（同上，跨服务器中继留待后续里程碑）。
...
- 打开：AA 客户端识别链接 → 解析地址阶梯（mDNS → 落库最后地址 → payload 里的 `host_server` 直接 Lookup → 本机自己配置的服务器 Lookup）→ 连接阶梯建连（直连/打洞/中继）→ 出示 token → 按权限取文件。**跨服务器的打洞/中继信令目前仍只会打向本机自己配置的服务器**（真正的跨服务器信令联邦是独立的、后置的项目，见 §12「多服务器联邦」），payload 携带的 `host_server` 目前只用于地址解析阶梯里的一次额外 Lookup 尝试，不会让打洞/中继也换目标服务器——分享方与打开方使用不同服务器时，只有二者恰好互为已配对设备（在对方允许名单里）时这一档才生效。
...
- `devices` 增列 `server_hint TEXT`：对端 home server 地址（含指纹），配对时交换（`PairServerHint`，proto ≥ 5，V0.3 迁移加列，线路层交换见 PROTOCOL.md §17）。同 `last_addr` 的新鲜度惯例——只在配对时写一次，不做持续刷新，需要更新时重新配对即可。
...
| 寻址 | 查谁去谁的 home server（地址查询已实现，见下）；信令/中继目前仍用**本机自己配置**的服务器；对端 server_hint 为 `devices` 正式字段（**已建表且配对时交换**，见下） | §3.4 |
...
| server_hint 交换（V0.3 遗留 gap 已补完） | 配对时新增 `PairServerHint` 追加变体（proto ≥ 5，不修改既有 `PairRequest`/`PairAccept`/`DeviceInfo`，同 `ResumeReport` 的既有原则）双向交换双方当前配置的 `server_url`；`resolve_addr` 新增一档查对端自己的服务器，不再局限于"自己的多台设备共用同一服务器"。仍是缩小范围：只解决地址查询，跨服务器中继/打洞信令联邦留待后续（见"多服务器联邦"） | §3.4，PROTOCOL.md §17 |
...
仍待实现阶段细化：打洞成功率的真实双网络验证（回环/CI 只能验证接线正确，不能验证真实穿透率）、单任务多流优化、`aa4c://` deep-link 系统级注册（桌面三平台 + Android intent）、分享二维码生成、读写分享（后置）、多服务器联邦（后置，含跨服务器中继/打洞信令——地址查询已随 `server_hint` 交换补完，见 §3.4/§12，但信令联邦仍需服务器间协议，是独立更大的项目）、iOS 后台长连接受限的退化方案。
