...
> 最后更新：2026-07-10（`v0.4.0-preview.1` 已发布：修复 `v0.4.0-preview` 的 Windows 安装包问题——choco shim 导致打包进去的 aria2c.exe 实际跑不起来，已改成直接下官方二进制并重新出包；除 Windows 安装包外没有任何功能/代码变化。**macOS 仍只有 arm64**（`engines.yml` 引擎自建流水线还没验证过），Intel Mac 用户仍需等待；CI 全绿（`quic_resume_after_disconnect`/`two_cores_pair_then_transfer` 那两条已知并行环境 flaky 除外，单线程下全过）。下一步是决策点：D2 / D3 / 引擎流水线首跑，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
整个 V0.1 桌面端链路 **发现 → 配对 → 传输 → UI** 已全部打通。**V0.3「AA Connect」六个里程碑（C1–C6）全部完成**：广域网 QUIC 会话层、自建信令+中继服务器、远程同步/发送接入完整连接阶梯、NAT 打洞、分享链接，一整条「局域网直连 → 公网直连 → 打洞 → 中继」的连接阶梯贯通，外加脱离设备配对关系的能力型分享。**V0.4「Download」里程碑 D1（Aria2/HTTP-FTP 下载）已实现，且已打包发布 `v0.4.0-preview` → 修复重发为 `v0.4.0-preview.1`**——新 crate `aa4c-download`、下载页可用、真实 `tauri dev` 走查跑通（sidecar 拉起、Tauri capability 权限、`stop-with-process` 孤儿进程防护均实测有效）；D2（Transmission/BT，v3 设计修订从 qBittorrent 换引擎）与 D3（任务中心打磨）仍是设计稿。⚠️ 这个 preview 的 macOS 安装包**只有 arm64**（引擎二进制自建流水线 `engines.yml` 还没验证过，`release.yml` 临时改用系统包管理器 aria2c 打包，见第四节的优先级最高遗留项）。**V0.2 同步五个里程碑（信任分级 / 本地索引 + Inbox / 跨设备索引交换 + 统一视图 / 按需拉取 / 冲突标记）全部落地**（SYNC_DESIGN.md §10）；线路协议已升到 `proto=2` 并对同步路径按版本 gate（与 v0.2.0-preview 的同步不再互通，趁预发布窗口对齐）。**真机 GUI 走查已人工跑通**（`scripts/dev-two-nodes.sh` 起两实例：配对 → 互标我的设备 → 黄「可下载」→ 点黄拉取转绿 → 同名不同内容「多版本」并列，均正常）。
...
## 四、下一步：`v0.4.0-preview.1` 已发布（Windows 修复重发，macOS 仍只有 arm64），等用户选下一步方向
...
**V0.4「Download」里程碑 D1（Aria2 集成，HTTP/HTTPS/FTP 直链下载）已实现，已打包发布 `v0.4.0-preview` → 修复重发为 `v0.4.0-preview.1`**：新 crate `aa4c-download`（不依赖 Tauri）+ `SidecarSpawner` 依赖倒置（Tauri 壳层注入，Core 保持纯 Rust）；新表 `download_tasks`（迁移 `008_downloads.sql`）；「下载」页可用（链接输入 + 任务列表 + 暂停/继续/取消/打开文件夹）。已过真实 `tauri dev` 走查：sidecar 拉起、Tauri capability 权限配置、`stop-with-process` 孤儿进程防护均实测有效（细节见 [DOWNLOAD_DESIGN.md](DOWNLOAD_DESIGN.md) §3.5「D1 实现偏差」）。**受限于本环境没有原生 GUI 自动化工具，走查没有覆盖实际点击「暂停/继续/打开文件夹」按钮这几步**——底层命令行为已由 `aa4c-download` 的 6 条真实 aria2c 集成测试覆盖，但如果你本人在桌面上快速点一遍这几个按钮，能补上这最后一段信心。

**✅ 已解决——`v0.4.0-preview` 的 Windows 安装包问题**：切这个 preview 时 `release.yml` 用 `choco install aria2` 给 Windows 装 aria2c，事后发现 choco 装的其实是个转发器 shim、不是真正的可执行文件（细节见第五节教训）——构建本身没报错（复制一个文件不需要执行它），但打进安装包里的 `aria2c.exe` 在真实用户机器上运行下载功能时会失败。`main` 上已经修好（改成直接下官方二进制）；用户决定不移动已发布的 tag，而是切一个新的 `v0.4.0-preview.1` 重新发布——已完成（见第一节进度表）。macOS/Linux 安装包不受这个问题影响（choco 只用在 Windows），未重新发布，仍是同一份产物。

**✅ 已解决——`engines.yml` 已跑通验证，`release.yml` 恢复正式状态**：手动触发 `workflow_dispatch`（`aria2_version=1.37.0`）跑了四轮才全绿，过程中连续踩了四个坑（详见第五节教训）：①macOS x86_64 在 arm64 runner 上用 `clang -arch x86_64` 交叉编译，链接找不到 Homebrew 装的 arm64 单架构依赖库符号——改成用原生 runner 各自编译；②改用的 `macos-13` 标签早已于 2025-12-04 退役，job 无限期排队排不到 runner——换成 `macos-15-intel`；③Linux 静态链接缺 `autopoint`（Ubuntu 上和 `gettext` 是两个独立包）；④Linux 静态链接缺 `liblzma-dev`，且 `libc-ares-dev` 在 Ubuntu 上不带静态 `.a`——补了前者，后者直接 `--without-libcares` 放弃（DNS 解析退化成同步阻塞，对 sidecar 用途无影响）。四个产物 + `SHA256SUMS` 已发到 `engines/aria2-1.37.0` release，校验和已填进 `scripts/fetch-engines.sh` 的 `checksum_for()`（本机用真实下载+校验模式跑通验证过，不是 `--from-path`）；顺手还修了 `fetch-engines.sh` 自己的一个 bug（正式下载分支覆盖 `--from-path` 模式留下的只读文件会写失败，加了 `rm -f` 再下载）。`release.yml` 已改回：macOS 目标 `universal-apple-darwin`、三平台 sidecar 都走 `fetch-engines.sh <triple>` 真实校验和下载（不再需要 brew/apt/choco 装 aria2，也不再需要 Windows 官方 zip 下载步骤——那一套只用于 `ci.yml` 跑真实测试，`release.yml` 现在只是复制校验过的二进制，不执行它）。**尚未验证的是下一次真实 tag push 触发的 `release.yml` 本身**——改动只在本机和 `engines.yml` 里验证过，`release.yml` 从没在这些新代码路径上真跑过一次，下次发布时要盯着看。

**D1 遗留的下一步选项**（不要凭本文档自己假设该做哪一个，直接问用户）：
- **D2（Transmission/BT-Magnet，v3 设计修订）**：BT 引擎已从 qBittorrent 换成 Transmission——按 D1 教训先逐项核实了三平台 headless 分发（qBittorrent 在 Windows/macOS 均无 nox 先例、Windows 完全空白；Transmission 官方 MSI 自带 daemon、Homebrew core formula 双架构齐全），设计定稿在 DOWNLOAD_DESIGN.md §3.6，实现要点在 V0.4_IMPLEMENTATION_PLAN.md D2。**编码前必做的小样验证**：孤儿进程防护（Transmission 没有 `stop-with-process` 等价物，Windows Job Object / Linux PDEATHSIG / macOS PID 清扫组合，见 §3.6.2）。另注意 v3 同时预留了 Lua 插件系统的设计边界（DOWNLOAD_DESIGN.md §10）——D2 实现 `add_download` 时必须走引擎无关请求描述中间层，这是插件钩子的预留接缝，不是可省略的间接层。
