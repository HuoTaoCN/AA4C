...
> 最后更新：2026-07-08（`v0.3.0-preview` 已发布：V0.3「AA Connect」全部完成，随三平台安装包/APK/服务器二进制发出；V0.4「Download」设计已评审修订至 v2，待用户确认后产出实现计划）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
## 四、下一步：`v0.3.0-preview` 已发布，V0.4 设计已修订至 v2，待用户确认后产出实现计划
...
**V0.4「Download」（统一下载中心）设计已评审修订至 v2**：[DOWNLOAD_DESIGN.md](DOWNLOAD_DESIGN.md)（§9 是已确认决策表；文档头部有 v1→v2 修订摘要）。三个范围决定已经用户确认：AA4C **自动打包并管理**外部下载引擎子进程（不要求用户自己装）；先做 **Aria2（HTTP/HTTPS/FTP）**，qBittorrent（BT/Magnet）后置为独立里程碑 D2；V0.4 **只覆盖桌面三平台**，不含 Android。核心架构决定：新 crate `aa4c-download`，用 `SidecarSpawner` trait 把"拉起子进程"这个 Tauri 专属能力（`tauri-plugin-shell`）与纯 Rust 的 Core 解耦（同 C1–C6 一路建立的依赖倒置先例）；新表 `download_tasks`**不复用** `transfer_tasks`（下载没有对端设备，规避 C6 踩过的外键假设冲突）。v2 评审新增的关键决定：引擎二进制自建构建流水线（官方 release 没有 macOS/Linux 产物）、任务跨重启恢复（`save-session` + 启动对账）、密钥走 0600 conf 文件、`stop-with-process` 孤儿进程防护、默认下载目录必须在 save_dir 子树之外。

**下一步等用户确认 v2 修订**：
- 用户过一遍 v2 修订内容（尤其文档头部列的 4 个实质问题的修法）没有异议后，产出 `V0.4_IMPLEMENTATION_PLAN.md` 把 D1 细化到可直接执行，然后开始实现。
- 或者先补一些 V0.3 范围内的已知缺口（见下），巩固刚发出去的 preview。
...
V0.3「AA Connect」六个里程碑（C1 QUIC + 断点续传、C2 `aa4c-server` 信令面、C3 Relay 中继、C4 远程同步/发送 + 连接质量、C5 NAT 打洞、C6 分享链接）**全部实现并测试通过，且已打包发布 `v0.3.0-preview`**——连接阶梯「局域网直连 → 公网直连 → 打洞 → 中继」四档全部贯通，外加脱离配对关系的能力型分享，随三平台安装包/APK/服务器二进制发出。**V0.4「Download」设计已评审修订至 v2**（[DOWNLOAD_DESIGN.md](DOWNLOAD_DESIGN.md)，头部有 v1→v2 修订摘要），尚未实现。**下一步等用户确认 v2**：确认后产出 `V0.4_IMPLEMENTATION_PLAN.md` 再开始实现；或按第四节先补 V0.3 已知缺口——不要凭 HANDOFF 文档自己假设该做哪一个，直接问用户。
