...
> 最后更新：2026-09-11（**V0.7「Trust / Reach」R1–R4 全部已实现并随 `v0.7.0-preview.1` 发布**——解决「家里的电脑和单位的电脑永远配不上对」这个死结，选定「引荐 + 一次用户确认」而非 Syncthing 式自动引荐，顺带打通了全链路 IPv6、UPnP 自动端口映射、桌面端内置服务器。**但 V0.7 的跨网部分至今没有真机验证过**——代码写完了，「两台在不同网络的设备真的连上了」还没被证实，清单在 [docs/V0.7_VERIFICATION.md](docs/V0.7_VERIFICATION.md)。**另一件必须先说的事：CI 从 2026-08-12 起再次转红，红了 29 天没人发现**——上一版 CHANGELOG 写的「CI 现在是绿的」只在写下的那一刻成立。这是同一个坑第二次踩（上次红了一周）。下一步与全部候选方向见第四节。**V0.5「AI」已随 `v0.5.0-preview` 发布**；**V0.6「Touch/Direct」设计稿已定稿，尚未开始实现**——见 [TOUCH_DESIGN.md](TOUCH_DESIGN.md) + [V0.6_IMPLEMENTATION_PLAN.md](V0.6_IMPLEMENTATION_PLAN.md)，关键结论：Android 经典 NFC P2P（Android Beam）已在 Android 10 弃用 / Android 14 移除，「碰一碰」必须走 HCE（需要自定义原生插件，官方 `tauri-plugin-nfc` 不支持）；**AA Touch 与 AA Direct 的 WiFi Direct 分支仅限 Android**；蓝牙分支 Android 双向对等、桌面仅 central；蓝牙 Mesh 明确后置。**本环境没有任何 NFC/WiFi Direct/蓝牙硬件，也没有真实 Android 设备**——V0.6 的核心功能只能由用户在真机上验证，这条写进了 V0.6_IMPLEMENTATION_PLAN.md 的执行纪律第 2 条。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
**V0.7「Trust / Reach」R1–R4 已全部实现并发布 `v0.7.0-preview.1`。**

**先说一件必须先处理的事：CI 又红了，而且红了 29 天。** `v0.7.0-preview.1` 的 CHANGELOG 写着
「CI 现在是绿的」——那句话在写下的那一刻是真的，但紧接着的两次 main 推送就把它推翻了，此后一直红着：

- run `31578582868`（`chore(release): 0.7.0-preview.1`）：**Windows** 上 `quic_roundtrip_transfer`
  panic `transfer failed (b side): io error: connection lost`。与 `quic_resume_after_disconnect`
  同一类的并行资源争抢——QUIC 的 keep-alive 2s / 空闲超时 8s 只容得下 3 次连丢的 PING。
- run `31579123088`（`Merge branch 'docs/align-v0.7'`）：**macOS** 下载 GGUF 测试固件时
...
> ⚠️ 版本兼容：proto 现为 **6**（`v0.7.0-preview` 起，`Message::IntroduceRequest`/`IntroducePeers`，V0.7 R2 信任传递；5 为配对时交换 `server_hint`，4 为 C6 的 `ShareRequest`——各档对应的章节列在 [PROTOCOL.md](PROTOCOL.md) 头部的版本表）；与更旧对端握手自动协商降级，行为不变（不发送对方不认识的高版本消息）。`v0.2.0-preview.2` 起的构建可与本版本互通同步；与 v0.1.x 仍因 `DeviceInfo.trust_level` 无法配对。
>
