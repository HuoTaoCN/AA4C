...
> 最后更新：2026-07-18（**V0.4「Download」里程碑 D2（Transmission/BT-Magnet）完整落地，含引擎二进制正式打包分发管线，已发布 `v0.4.0-preview.2`**：`engines.yml` 新增 transmission-windows/macos/linux/publish 四个 job，产物发布到 `engines/transmission-4.1.3`；`scripts/fetch-engines.sh` 学会 Transmission "每个三元组一个 zip"的产物形态；`tauri.conf.json` 新增第二个 `externalBin` + `bundle.resources`；桌面壳层 `TauriSidecarSpawner` 在拉起 transmission-daemon 前注入 `DYLD_LIBRARY_PATH`/`LD_LIBRARY_PATH`（`externalBin` 与 `bundle.resources` 打包后落在不同目录，查证 tauri-bundler 源码确认，纯运行时修复不用碰 CI）。**真机 CI 验证中发现并修复两个真实 bug**：Linux 的"隔离目录冒烟测试"其实没有真正隔离（系统里还留着构建依赖，ld.so 会兜底找到）；依赖库打包脚本的排除清单是不锚定边界的子串匹配，`libc`/`libm` 意外排除了 `libcurl`/`libminiupnpc`——两处均已修复并重新发布。详见第五节教训与 CHANGELOG.md。V0.4「Download」D1+D2 至此全部实现并已随三平台安装包正式发布。下一步是决策点：D3（任务中心打磨）还是别的方向，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
## 四、下一步：V0.4 里程碑 D1+D2 均已完整实现并发布，等用户选下一步方向
...
**D3（任务中心打磨）候选内容**（下一步该不该做这个，交给用户决定）：设置页下载区块（目录/限速/并发/分享率）、下载目录落在同步范围内的警示交互、批量操作、D1+D2 任务统一排序（数据模型上已经是同一张表，UI 排序目前也已经是按 `created_at` 统一的，D3 主要是打磨而非从零搭）。或补 V0.3 已知缺口（见下）——都不冲突，可穿插进行。
