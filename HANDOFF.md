...
> 最后更新：2026-07-09（`v0.4.0-preview` 已发布：V0.4「Download」里程碑 D1 随三平台安装包 + Android arm64 APK 发出——**macOS 只有 arm64**，且**已发布的 Windows 安装包大概率打包了一个跑不起来的 aria2c**（choco shim 问题，已在 main 修复，tag 未重新打包），见第四节待决问题；CI 全绿（`quic_resume_after_disconnect` 那条已知 flaky 除外）；下一步是决策点：先处理 Windows 安装包 / D2 / D3 / 引擎流水线首跑，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
**⚠️ 待决问题——已发布的 `v0.4.0-preview` 的 Windows 安装包需不需要重新发**：切这个 preview 时 `release.yml` 用 `choco install aria2` 给 Windows 装 aria2c，事后发现 choco 装的其实是个转发器 shim、不是真正的可执行文件（细节见第五节教训）——构建本身没报错（复制一个文件不需要执行它），但打进安装包里的 `aria2c.exe` 在真实用户机器上运行下载功能时会失败。`main` 上已经修好（改成直接下官方二进制，见 CI 修复过程），但**这个已经发布、有人可能已经下载的 tag 本身还没有重新打包**——要不要现在就重新触发一次 release（可以移动 tag 到修复后的 commit 重新打包，或者直接切一个 `v0.4.0-preview.1`）是一个需要用户决定的事，不要自己假设该怎么处理。macOS/Linux 安装包不受这个问题影响（choco 只用在 Windows）。

**优先级次之——`engines.yml` 从未真实验证过，`release.yml` 目前是临时状态**：这次 preview 的 macOS 安装包只有 arm64（不含 Intel），原因是 `.github/workflows/engines.yml`（下载引擎二进制自建流水线）从没真正跑过一次，`scripts/fetch-engines.sh` 的校验和是空的，`release.yml` 只能临时改用 `--from-path`（CI runner 装的包管理器版本 aria2c 顶位，不校验）——而 GitHub Actions 的 macOS runner 是 arm64，拿不到 x86_64 版本，所以 `release.yml` 的 macOS 构建目标也临时从 `universal-apple-darwin` 改成了 `aarch64-apple-darwin`（两处改动都标了 `TODO(engines.yml 验证后恢复)` 注释，一搜就能找到）。**在下一个版本发布之前，应该找时间手动触发一次 `engines.yml`**（GitHub Actions 页面 workflow_dispatch），大概率第一次跑要按报错调整构建命令（写作时只能依据 aria2 官方 README 推断，未经真实验证，风险已写在 workflow 注释里）；跑通后把产出的 `SHA256SUMS` 填进 `scripts/fetch-engines.sh` 的 `checksum_for()`，再把 `release.yml` 那两处 TODO 改回真正的校验和下载 + universal 构建。
