...
> 最后更新：2026-07-28（**V0.5「AI」里程碑 AI1（规则式归档，无 AI）已实现**：新类别识别（扩展名+~20 条 magic bytes）、纯 Rust 手写 GGUF 头解析（`general.file_type` 量化枚举表对照 llama.cpp `master` 源码逐项核实，不是猜的）、按 `position` 顺序匹配的规则引擎（目标目录模板 7 种占位符、`rename`失败回退拷贝、同名加序号）、下载完成后台钩子（只挂 `DownloadDone`，不给同步/传输收件自动归档）、撤销（挪回原位+摘标签）；五条预设规则默认停用，归档页给"一键启用推荐规则"。新表 `archive_rules`/`archive_entries`/`archive_tags`/`archive_log`（`009_archive.sql`）、7 个 Tauri Command、归档页与设置页「归档」区块均已落地并在 Browser 预览里人工走查确认渲染正确。新增 30+ 条测试（含真实文件系统移动/撤销 + 走 `Core` 公开方法的全链路测试）。发布前 `cargo fmt`/`clippy --workspace -D warnings`/`cargo test --workspace`（单线程复核 `aa4c-core --test core`，`quic_resume_after_disconnect` 复现了本会话早前已确认的既有环境 flaky，非本次改动引入）/`pnpm build` 均验证通过。AI2–AI5（AI 引擎/建议/知识库/发布）仍是设计稿，下一步从 AI2 继续，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
## 四、下一步：V0.5「AI」里程碑 AI1（规则式归档）已实现，从 AI2（llama-server 引擎接入）继续
...
**V0.5「AI」里程碑 AI1（规则式归档，无 AI 即完整可用）已实现**（本次会话，见第一节表格）：设计见 [ARCHIVE_DESIGN.md](ARCHIVE_DESIGN.md)（其 §10 是已确认决策表、§11 是 AI2 动手前必须补的实证清单——**AI2 动手前先读一遍**），实现计划见 [V0.5_IMPLEMENTATION_PLAN.md](V0.5_IMPLEMENTATION_PLAN.md)（里程碑 **AI1–AI5**——"AI"前缀是 V0.5 代号，与本文档下方历史章节「A1 已完成要点（Android 适配）」里的 Android 里程碑编号 A1 无关，起名时刻意避开了撞车）。**下一步是 AI2（`aa4c-engine` sidecar 设施重构 + llama-server 接入 + 打包腿 + 模型库）**，动手前先做 V0.5_IMPLEMENTATION_PLAN.md AI2.0 列的前置实证（锁定 llama.cpp tag、三平台产物裁剪清单、微型 GGUF 测试夹具），不要凭 §3.1 已有的规划期实证直接开始写代码——那批实证是"证明方案可行"，AI2.0 这批是"钉死这一次实现具体用的版本/文件"，两码事。也可穿插补 V0.3 其余已知缺口（见下）。
