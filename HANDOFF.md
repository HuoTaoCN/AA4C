...
**⚠️ 待办——Linux AppImage 打包在 GitHub Actions `ubuntu-latest`（24.04）上失败，`v0.4.0-preview.2` 的 Linux 安装包这次只有 `.deb`/`.rpm`，没有 `.AppImage`**：`tauri build` 报 `failed to bundle project 'failed to run linuxdeploy'`，Tauri 侧不透出任何底层 stderr；`.deb`/`.rpm` 每次都能正常打包，只有 appimage 这一个 bundle 目标炸，说明应用本身构建和 D2.7/D2.8 的引擎二进制打包都没问题。已经试过两个标准方案都没解决：① `apt install libfuse2`（Ubuntu 24.04 默认只带 FUSE3，装了之后确认 apt 解析成 `libfuse2t64` 成功安装，但报错原样重现）；② `APPIMAGE_EXTRACT_AND_RUN=1`（让 AppImage 走"解包后直接跑"绕开 FUSE 挂载，同样没用）。`release.yml` 目前 `ubuntu-latest` 的 `args` 临时改成 `--bundles deb,rpm` 跳过 appimage 目标（有 `TODO` 注释标记）。真正根因没查清楚——可能是 GitHub Actions 容器化 runner 压根不给非特权进程挂载权限（不只是缺库），需要真的登进一个类似环境手动跑 `linuxdeploy-x86_64.AppImage --appimage-extract` 之类的命令看实际报错，而不是隔着 Tauri 这层再猜。下次要恢复 AppImage 输出，从这里查起。
