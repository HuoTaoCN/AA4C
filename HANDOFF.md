...
> 最后更新：2026-07-09（`v0.4.0-preview` 已发布：V0.4「Download」里程碑 D1 随三平台安装包 + Android arm64 APK 发出——**macOS 这次只有 arm64，不含 Intel**，见下方说明；下一步是决策点：D2/D3/引擎流水线首跑，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
整个 V0.1 桌面端链路 **发现 → 配对 → 传输 → UI** 已全部打通。**V0.3「AA Connect」六个里程碑（C1–C6）全部完成**：广域网 QUIC 会话层、自建信令+中继服务器、远程同步/发送接入完整连接阶梯、NAT 打洞、分享链接，一整条「局域网直连 → 公网直连 → 打洞 → 中继」的连接阶梯贯通，外加脱离设备配对关系的能力型分享。**V0.4「Download」里程碑 D1（Aria2/HTTP-FTP 下载）已实现，且已打包发布 `v0.4.0-preview`**——新 crate `aa4c-download`、下载页可用、真实 `tauri dev` 走查跑通（sidecar 拉起、Tauri capability 权限、`stop-with-process` 孤儿进程防护均实测有效）；D2（qBittorrent/BT）与 D3（任务中心打磨）仍是设计稿。⚠️ 这个 preview 的 macOS 安装包**只有 arm64**（引擎二进制自建流水线 `engines.yml` 还没验证过，`release.yml` 临时改用系统包管理器 aria2c 打包，见第四节的优先级最高遗留项）。**V0.2 同步五个里程碑（信任分级 / 本地索引 + Inbox / 跨设备索引交换 + 统一视图 / 按需拉取 / 冲突标记）全部落地**（SYNC_DESIGN.md §10）；线路协议已升到 `proto=2` 并对同步路径按版本 gate（与 v0.2.0-preview 的同步不再互通，趁预发布窗口对齐）。**真机 GUI 走查已人工跑通**（`scripts/dev-two-nodes.sh` 起两实例：配对 → 互标我的设备 → 黄「可下载」→ 点黄拉取转绿 → 同名不同内容「多版本」并列，均正常）。
...
## 四、下一步：`v0.4.0-preview` 已发布（macOS 只有 arm64），等用户选下一步方向
...
**V0.4「Download」里程碑 D1（Aria2 集成，HTTP/HTTPS/FTP 直链下载）已实现，已打包发布 `v0.4.0-preview`**：新 crate `aa4c-download`（不依赖 Tauri）+ `SidecarSpawner` 依赖倒置（Tauri 壳层注入，Core 保持纯 Rust）；新表 `download_tasks`（迁移 `008_downloads.sql`）；「下载」页可用（链接输入 + 任务列表 + 暂停/继续/取消/打开文件夹）。已过真实 `tauri dev` 走查：sidecar 拉起、Tauri capability 权限配置、`stop-with-process` 孤儿进程防护均实测有效（细节见 [DOWNLOAD_DESIGN.md](DOWNLOAD_DESIGN.md) §3.5「D1 实现偏差」）。**受限于本环境没有原生 GUI 自动化工具，走查没有覆盖实际点击「暂停/继续/打开文件夹」按钮这几步**——底层命令行为已由 `aa4c-download` 的 6 条真实 aria2c 集成测试覆盖，但如果你本人在桌面上快速点一遍这几个按钮，能补上这最后一段信心。

**⚠️ 优先级最高的遗留项——`engines.yml` 从未真实验证过，`release.yml` 目前是临时状态**：这次 preview 的 macOS 安装包只有 arm64（不含 Intel），原因是 `.github/workflows/engines.yml`（下载引擎二进制自建流水线）从没真正跑过一次，`scripts/fetch-engines.sh` 的校验和是空的，`release.yml` 只能临时改用 `--from-path`（CI runner 装的包管理器版本 aria2c 顶位，不校验）——而 GitHub Actions 的 macOS runner 是 arm64，拿不到 x86_64 版本，所以 `release.yml` 的 macOS 构建目标也临时从 `universal-apple-darwin` 改成了 `aarch64-apple-darwin`（两处改动都标了 `TODO(engines.yml 验证后恢复)` 注释，一搜就能找到）。**在下一个版本发布之前，应该找时间手动触发一次 `engines.yml`**（GitHub Actions 页面 workflow_dispatch），大概率第一次跑要按报错调整构建命令（写作时只能依据 aria2 官方 README 推断，未经真实验证，风险已写在 workflow 注释里）；跑通后把产出的 `SHA256SUMS` 填进 `scripts/fetch-engines.sh` 的 `checksum_for()`，再把 `release.yml` 那两处 TODO 改回真正的校验和下载 + universal 构建。
...
- **Windows 上 `choco install X` 装完之后，同一个 job 里默认 shell（`pwsh`）不一定马上看得到新装的可执行文件，哪怕紧接着的 bash 步骤能看到**（D1 教训，切 `v0.4.0-preview` 时在真实 CI 里踩到）：CI 里先 `choco install aria2 -y`，再用一个显式 `shell: bash` 的步骤跑 `scripts/fetch-engines.sh --from-path`——这一步成功了，说明 bash 视角里 `aria2c.exe` 确实在 PATH 上。但紧接着 `cargo test --workspace`（没写 `shell:`，Windows 上默认走 `pwsh`）却在下载端到端测试里报 `Unavailable("download engine not available")`——`ProcessSpawner` 内部按 PATH 搜索 `aria2c.exe` 找不到，说明 `cargo test` 生成的测试子进程继承的 PATH 视图和刚才 bash 那一步不是同一份。**排查方法**：先看失败测试报的错误（`Unavailable`，不是 panic/超时），定位到是 `require_aria2c()` 的 PATH 搜索没找到东西，而不是 aria2 本身没装成功——因为紧邻的上一步（bash 里跑 fetch-engines.sh）明明成功过，说明"装没装成功"不是问题，"这一步看不看得到"才是问题，缩小了排查范围。**修法**：给 `cargo test --workspace` 那一步也显式加 `shell: bash`，让它和验证过确实能看到 aria2c 的那个 shell 保持一致，不依赖 GitHub Actions 每个 `run:` 步骤默认 shell（Windows 是 `pwsh`，其余平台是 `bash`）之间可能存在的 PATH 传播时机差异。**没深究的部分**：没有去确认这是不是 GitHub Actions 托管 runner 上 chocolatey 的已知通病，还是这次运行偶然踩到的时序问题——反正统一用 bash 之后行为是确定的，没必要为了解释一个第三方工具链的内部机制而过度investigate。
