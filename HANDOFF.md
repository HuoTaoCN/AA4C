...
> 最后更新：2026-07-20（**`v0.4.0` 正式 GA 已发布并真机 CI 验证成功**：V0.4「Download」四个里程碑（D1 Aria2/HTTP-FTP、D2 Transmission/BT-Magnet、D2 收尾引擎打包管线、D3 统一任务中心打磨）代码与发布双双完整——D1+D2 此前已随 `v0.4.0-preview.2` 打包发布，这次把 D3 一起带上；同时把 V0.3 遗留的跨服务器好友寻址 gap（配对时交换 `server_hint`，`PROTO_VERSION` 4→5）也一并纳入这一版。`gh run view` 确认 5 个 job 全绿（Windows/macOS/Linux 三平台安装包 + Android APK + `aa4c-server` Linux 二进制），GitHub Release 非 draft/非 prerelease，9 个产物齐全，含此前单独修过的 Linux `.AppImage`（这次真机验证随正式发布一起打出来了）。发布前 `cargo fmt`/`clippy -D warnings`/`cargo test --workspace`（单线程复核 `aa4c-core --test core`）/`pnpm build` 均验证通过。下一步：继续补 V0.3 其余已知缺口（`aa4c://` deep-link 系统级注册等）或开始规划 V0.5，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
## 四、下一步：`v0.4.0` 正式版已切出（V0.4「Download」D1+D2+D3 全部随其发布 + V0.3 跨服务器好友寻址 gap 已补完），等用户选下一步方向
