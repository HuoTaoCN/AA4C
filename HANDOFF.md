...
> 最后更新：2026-07-20（**V0.3 遗留 gap 补完：跨服务器好友寻址（配对时交换 `server_hint`）**——`devices.server_hint` 列自里程碑 C2 就建表，但配对协议一直没交换这个字段，两个用户各自搭独立 `aa4c-server` 互为朋友后找不到对方，是已知的、有意缩小的范围。这次补上：新增协议追加变体 `Message::PairServerHint`（`PROTO_VERSION` 4→5，不改既有 `PairRequest`/`PairAccept`/`DeviceInfo`，同 `ResumeReport` 的既有原则——bincode 位置编码，加字段会破坏旧版本解码），配对双方在 `PairConfirm` 互相确认之后、写库之前互相声明自己当前配置的 `server_url`；`resolve_addr`（`resolve_peer`/`sync_exchange` 共用的解析阶梯）新增一档查对端自己的服务器，`aa4c-server` 的 `Lookup` 鉴权只看目标设备自己的允许名单、不要求查询方也注册在那台服务器，服务器端不需要改一行代码。**明确不做**：跨服务器的中继/打洞信令联邦（两台互不知情的服务器没有公共撮合点，需要服务器间协议，是独立的、后置的项目）。新增测试：`aa4c-identity/tests/pairing.rs` 两条真实 TLS 双节点测试（双向交换、`enable_remote=false` 时不声明）+ `aa4c-core/tests/core.rs` 一条跨服务器 e2e（两个真实独立 `aa4c-server` 实例）。调试过程中意外发现这台开发机的沙箱环境有一块虚拟网卡被分配了裸网段地址 `192.168.97.0`（不是合法主机地址），会偶发导致这条新 e2e 测试的中继/打洞兜底必然失败——已确认是沙箱环境缺陷、不是代码逻辑问题，记录在测试的文档注释里，真实用户机器/CI 不会有这种网卡。发布前 `cargo fmt`/`clippy -D warnings`/`cargo test --workspace` 均验证通过。V0.4「Download」D1+D2+D3 的历史记录见下方进度表；Linux AppImage 修复见同表。下一步：V0.4 正式发布，还是继续补其余 V0.3 已知缺口（`aa4c://` deep-link 注册等），见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
## 四、下一步：V0.4「Download」D1+D2+D3 均已完整实现（D3 待打包发布）+ V0.3 跨服务器好友寻址 gap 已补完，等用户选下一步方向
...
**V0.4 里程碑 D3（统一任务中心打磨）已完整实现**：设置页「下载」区块（目录+同步范围警示、限速、并发、BT 分享率/做种超时）、批量操作（全部暂停/全部继续/清除已完成记录）、下载失败原因人话转译均已落地并通过 `cargo test --workspace`/`pnpm build` 验证，细节见第一节表格。**尚未随新版本打包发布**——V0.4「Download」四个里程碑（D1/D2/D2 收尾/D3）代码层面已全部完成，下一步是决策点：现在切一个新版本（如 `v0.4.0` 正式版或 `v0.4.0-preview.3`）打包发布，还是先补 V0.3 已知缺口（见下）——都不冲突，可穿插进行。

**跨服务器好友寻址 gap 已补完**（本次会话新增，见第一节表格）：配对时交换 `server_hint`，`resolve_addr` 能查对端自己的服务器，不再局限于「自己的多台设备」+「双方恰好用同一服务器」两种场景。**仍未做、明确缩小的范围**：中继的 `RelayDialer`/打洞的 `PunchDialer` 依然只连自己配置的服务器——跨服务器的中继/打洞信令联邦需要服务器间协议，是独立的、更大的项目（CONNECT_DESIGN.md §12「多服务器联邦」），本次不做。
