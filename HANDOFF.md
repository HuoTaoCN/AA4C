...
> 最后更新：2026-08-02（**V0.5「AI」里程碑 AI1（规则式归档）+ AI2（llama-server 引擎接入）+ AI3（AI 标签/分类建议）全部完成**——AI3.1 新模块 `aa4c-ai::suggest`（单并发批量队列 + JSON Schema 约束输出）→ AI3.2 `Core` 编排（`start_suggest`/`list_suggestions`/`resolve_suggestion` + `AiSuggestProgress` 事件）→ AI3.3 归档页新增「AI 建议」分区 → AI3.4 集成测试（`aa4c-ai` 一个 + `Core` 编排层一个真实模型端到端测试，覆盖"选文件→出建议→采纳→文件真的被移动/打标签/可撤销"全链路）。`cargo test --workspace`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo fmt --all --check`/`pnpm build` 全部通过（单线程复核排除已知 QUIC 并行资源争抢 flaky 的干扰，与本次改动无关）。全部细节见 [ARCHIVE_DESIGN.md](ARCHIVE_DESIGN.md) §5/§3.7。下一步是 AI4（本地知识库）或用户指定方向，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
**V0.5「AI」里程碑 AI1（规则式归档）+ AI2（llama-server 引擎接入）+ AI3（AI 标签/分类建议）均已实现**（见第一节表格）：设计见 [ARCHIVE_DESIGN.md](ARCHIVE_DESIGN.md)（其 §10 是已确认决策表），实现计划见 [V0.5_IMPLEMENTATION_PLAN.md](V0.5_IMPLEMENTATION_PLAN.md)（里程碑 **AI1–AI5**——"AI"前缀是 V0.5 代号，与本文档下方历史章节「A1 已完成要点（Android 适配）」里的 Android 里程碑编号 A1 无关，起名时刻意避开了撞车）。

**AI3 全部已完成并提交推送**（完整细节见 ARCHIVE_DESIGN.md §5/§3.7，这里只留摘要）：
- **AI3.1** 新模块 `aa4c-ai::suggest`：`SuggestEngine` 单并发批量队列 + `response_format.json_schema` 约束输出（AI2.0 实证的请求形态照抄，温度 0.2）；引擎不可用/超时该文件标"建议失败"不阻塞其余队列。
- **AI3.2** `Core` 新增 `start_suggest`/`list_suggestions`/`resolve_suggestion` 三个编排方法（同名 Tauri Command）+ `CoreEvent::AiSuggestProgress` 事件；`archive::engine::apply_suggestion()`（与规则式归档共享落库逻辑）实现采纳语义。
- **AI3.3** 归档页新增独立「AI 建议」分区（进度+待确认列表+采纳/忽略），沿用 AI1 建立的纵向堆叠分区风格，未按 §8 设计做成 tab。
- **AI3.4** 集成测试：`aa4c-ai::suggest` 单测覆盖请求构造/响应解析/降级路径，另有 2 个真实 `llama-server`+微型 GGUF 端到端测试（`aa4c-ai` 一个 + `aa4c-core::tests::core.rs` 新增的 `ai_suggest_lifecycle_through_core_orchestration` 一个，后者验证"选文件→出建议→采纳→文件真的被移动/打标签/可撤销"全链路）。`cargo test --workspace`（真实模型环境变量注入）单线程复核排除已知 QUIC 并行资源争抢 flaky 干扰（`quic_resume_after_disconnect`/`two_cores_pair_then_transfer` 隔离重跑均通过，与本次改动无关）；`cargo clippy --workspace --all-targets -- -D warnings`/`cargo fmt --all --check`/`pnpm build` 全部通过；Browser 预览人工走查确认「AI 建议」分区渲染正确。

**AI2 已知限制仍未解决**（AI3 未涉及，留给 AI5 发版时处理）：`release.yml` 的 AppImage 打包本身尚未被真实版本发布验证；推荐模型的 ModelScope/HF 直链未核实、未实现；真机 `pnpm tauri dev`（含真实 Tauri 后端 + 真实模型）走查未做，目前只验证了纯前端 dev 模式。

**下一步**：AI4（本地知识库）或用户指定方向。也可穿插补 V0.3 其余已知缺口（见下）。
