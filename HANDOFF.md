...
> 最后更新：2026-07-20（**V0.4「Download」里程碑 D3（统一任务中心打磨）完整落地——V0.4「Download」四个里程碑（D1 Aria2/HTTP-FTP、D2 Transmission/BT-Magnet、D2 收尾引擎打包管线、D3 任务中心打磨）至此全部实现**：设置页新增「下载」区块（下载目录+同步范围警示、限速、并发数、BT 分享率上限、BT 空闲做种超时，均真机核实过 Transmission 真实配置键名）；任务列表新增批量操作（全部暂停/全部继续/清除已完成记录，清除时先让引擎"忘记"任务再删 DB 行，避免 BT 做种任务被孤儿补插逻辑复活）；下载失败原因人话转译（aria2 `errorCode` 对照真机 `man aria2c` 核实的 EXIT STATUS 表）。发布前 `cargo fmt`/`clippy -D warnings`/`cargo test --workspace`（单线程 `aa4c-core --test core` 16/16 全过）/`pnpm build` 均验证通过。另外，`v0.4.0-preview.2` 发布时漏打的 Linux AppImage 也已单独查清根因（linuxdeploy 打包阶段自扫 ELF 依赖，跟运行时 `LD_LIBRARY_PATH` 注入是两回事）并修复，详见下方进度表与第五节教训。下一步是决策点：V0.4 正式发布，还是回头补 V0.3 遗留的跨服务器好友发现/深链接注册等 gap，见第四节）。用途：在新电脑上 `git clone` 后按本文档配置环境，即可无缝继续开发。
...
## 四、下一步：V0.4「Download」D1+D2+D3 均已完整实现，D3 待打包发布，等用户选下一步方向
