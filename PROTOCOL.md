...
|------|-----|
| 协议版本（V0.1） | `proto = 1` |
| 协议版本（V0.3 草案） | `proto = 2` |
| 默认端口 | 42420（TCP；v2 同端口 UDP 用于 QUIC/打洞） |
...
> ⚠️ 以下为设计草案，V0.3 开发前需评审定稿。原则：**v2 是 v1 的超集**，帧层、消息层、安全层不变，只扩展会话层与发现层。
...
- `Hello.proto` 协商取双方最小值；v2 端连 v1 端自动降级为 v1 行为
- mDNS TXT 的 `proto` 字段提前告知能力，避免无效尝试
- 新增消息只允许追加 enum 变体；v3 起引入 capability flags 做细粒度协商
