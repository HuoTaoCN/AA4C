...
- **引擎二进制正式打包分发管线已完成**：engines.yml 的 transmission-windows/macos/linux/publish 四个 job 全部跑通并发布到 `engines/transmission-4.1.3`（Windows 从 135 文件/58MB 的 msiexec 全量解包裁剪到 4MB 的 daemon+4 个真实运行时依赖，macOS/Linux 各自补齐一轮真实 CI 报错后收敛，过程见上方"已实现"记录）；`fetch-engines.sh` 按三元组下载+校验 zip、解包出 daemon 可执行文件（改名进 `binaries/`）与其余依赖库文件（进 `binaries/transmission-daemon-<triple>-libs/`，供下面的 `bundle.resources` 引用），`--from-path` 开发模式对应扩展（transmission-daemon 本体 best-effort、`-libs/` 占位目录总会建，见下一条）；`tauri.conf.json` 的 `externalBin` 加了 `binaries/transmission-daemon`，`bundle.resources` 用 `binaries/transmission-daemon-*-libs/*` glob 把依赖库一并打进包。
- **`externalBin` 与 `bundle.resources` 打包后落在不同目录，靠运行时注入库搜索路径解决，不是靠 CI 侧 rpath**：查证 tauri-bundler 源码（不是猜测）确认——Windows NSIS 把两者都装进同一个 `$INSTDIR`，天然同目录，无需额外处理；但 macOS `.app` 把 `externalBin` sidecar 放进 `Contents/MacOS/`、`bundle.resources` 放进 `Contents/Resources/`，Linux deb/AppImage 把 sidecar 放进 `usr/bin/`、resources 放进 `usr/lib/<ProductName>/`，两个平台都是不同目录。而 engines.yml 给 transmission-daemon 的 dylib/so 依赖打的 install name/`rpath`（`@executable_path/<lib>` / `$ORIGIN`）假设的正是"依赖库和可执行文件同目录"——这个假设在打包后的 macOS/Linux 应用里不成立，照抄 CI 那套会导致 sidecar 启动时找不到依赖库直接崩。修法二选一：（a）在 CI 侧把 rpath 改成指向 Resources/lib 的相对路径再重新构建发布，或（b）在 spawn 子进程时运行时注入库搜索路径环境变量。选了 (b)：`apps/desktop/src-tauri/src/download_spawner.rs` 的 `TauriSidecarSpawner::spawn` 在拉起 `transmission-daemon` 前，用 `app.path().resource_dir()` 算出 `bundle.resources` 落地的真实目录，macOS 上设 `DYLD_LIBRARY_PATH`/`DYLD_FALLBACK_LIBRARY_PATH`、Linux 上设 `LD_LIBRARY_PATH` 指向 `<resource_dir>/transmission-libs`（Windows 不需要，同目录天然找得到）——不用碰 engines.yml、不用重新发布 release、纯 Rust 代码改动，本地可测（`app.path().resource_dir()` 找不到或目录为空都不报错，交给 dyld/ld.so 自己失败，那层错误信息已经够诊断）。aria2c 是单文件静态二进制，不受影响，不注入。
- **`bundle.resources` 的 glob 在 `cargo build`/`check`/`test` 时也会被 `tauri-build` 校验、零匹配直接编译报错**——不是只在 `tauri build` 打包时才检查（本地实测确认：临时移走 `-libs/` 目录，`cargo check -p aa4c-desktop` 立刻报 `glob pattern ... path not found`）。这意味着 `--from-path` 开发模式必须始终产出一个非空的 `-libs/` 占位目录（哪怕 transmission-daemon 本体没找到），否则任何没跑过正式下载模式的贡献者本地 `cargo build --workspace` 都会炸——`fetch-engines.sh` 的 `--from-path` 分支已经处理（放一个空占位文件，dev 模式反正走系统库路径，用不到这个目录的实际内容）。
