...
> 状态：**设计初稿（v1，待评审）**，对应 [ROADMAP.md](ROADMAP.md) V0.4（AA Download）。本文档是落地依据，不含实现代码；实现拆解见后续 `V0.4_IMPLEMENTATION_PLAN.md`（评审通过后再产出，同 V0.3 CONNECT_DESIGN 先例）。
...
解法复用 C1–C6 里反复验证过的依赖倒置手法（`IncomingPairDispatch` / `RelayDialer` / `PunchDialer` / `ShareResolver` 都是同一个模式）：`aa4c-download` 定义一个 `SidecarSpawner` trait（"给我某个可执行文件的一个可读写句柄，我来跟它说 JSON-RPC"），具体实现由 Tauri 壳层注入，基于 `tauri_plugin_shell::ShellExt::sidecar()`。`aa4c-download` 自己不知道、也不需要知道背后是不是 Tauri——将来要在无 GUI 的 Docker/NAS 场景跑 headless Core，注入一个直接 `std::process::Command` 的实现即可，`aa4c-download` 内部逻辑不用改一行。
...
- **打包**：用 Tauri 2 的 sidecar 机制（`tauri.conf.json` 的 `bundle.externalBin`）。每个目标平台/架构各准备一份 aria2 官方发行二进制，按 `aria2c-<target-triple>[.exe]` 命名放进 `src-tauri/binaries/`（如 `aria2c-x86_64-pc-windows-msvc.exe`、`aria2c-aarch64-apple-darwin`、`aria2c-x86_64-unknown-linux-gnu`）。CI 新增一步：从 aria2 官方 release 下载对应平台产物并校验 checksum——不自己编译，直接用官方/受信构建，降低维护面。
- **启动**：`Core::start()` 阶段（与传输监听、mDNS 广播等其余服务同一批，见下方"eager vs lazy"的决定）通过注入的 `SidecarSpawner` 拉起：
  ```
  aria2c --enable-rpc --rpc-listen-port=<探测到的空闲端口>
         --rpc-listen-all=false --rpc-secret=<每次启动随机生成>
         --dir=<Settings.download_dir>
  ```
- **端口与密钥不持久化**：每次启动随机生成，只在内存里传给 `Aria2Client`——同 aa4c-server 中继 token 的思路一致（短生命周期凭证，用完即弃，缩小被动攻击面）。
- **关闭**：`Core::shutdown()` 时终止子进程——先礼后兵，`aria2.shutdown` RPC 优雅关闭，超时后强制 kill。
...
- `Settings` 追加 `download_dir: Option<String>`：默认落在 `save_dir` 同级的 `Downloads` 子目录，**与 AA 传输的接收目录分开**——下载的是不受信的公网内容，不应该默认混进"收到的"/任何同步范围，要不要同步给其他设备由用户自己决定（见 §7）。
...
| RPC 暴露面 | `--rpc-listen-all` 保持关闭（只绑 127.0.0.1），`--rpc-secret` 每次启动随机生成、只存内存——本机其他进程理论上能连到 aria2c 的回环端口，但拿不到密钥就调不了任何方法 |
...
| 子进程崩溃/被杀 | aria2c 异常退出时下载能力整体不可用，但不影响 AA4C 其余功能（同其余可选能力的一贯降级设计）；D1 先不做自动重启，观察实际故障率后再决定要不要加 |
...
| RPC 端口/密钥生命周期 | 不持久化，每次启动随机生成、只存内存——同中继 token 的短生命周期凭证思路 | §3.1，§7 |

## 仍待实现 / 后续

qBittorrent Web API 的具体鉴权细节（cookie session，需要单独设计）；限速/并发数等选项具体要透传 aria2/qBittorrent 的哪些 option、设置页字段怎么设计；下载失败的自动重试策略；子进程崩溃后的自动重启策略；Android 平台的下载能力方案（很可能是完全不同的技术路径，比如系统 DownloadManager，而不是 bundled 二进制，需要单独评估）；S3 协议支持（PROJECT_VISION.md 提到但未细化，大概率需要凭证管理，单独评估，不在 D1–D3 范围内）。
