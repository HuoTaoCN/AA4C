...
### 3.6 AI2.1/AI2.2/AI2.3 实现偏差（2026-07-30，代码已落地，模型库/前端仍未做）
...
- **未做**：`Settings`/Command/前端模型库（AI2.4）、`aa4c-core` 尚未实例化 `AiService`（Core 编排接线属于 AI2.4 的一部分，AI2.2/AI2.3 只交付独立可用的 crate + 打包管线）、`release.yml` 的 AppImage 产出与 macOS lipo 合并逻辑本身**尚未被真实 tag push 验证过**（只验证到 `cargo check -p aa4c-desktop` 本地通过 + `engines.yml` 独立跑通，release.yml 要等真实打 tag 发版才会触发，按计划这是 AI5 的事）。
